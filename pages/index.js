// pages/index.js — Step 5 complete: Evidence + Search + Live API cards (fetch* endpoints)
...
/* Server-side: preload tag lists + first page */
...
export default function Home({
  ...
  function buildQuery(supabase, countOnly = false) {
    ...
    if (like) {
      query = query.or(`title.ilike.%${like}%,summary.ilike.%${like}%,tags.ilike.%${like}%`);
    }
    return query;
  }
  ...
  function onAreaChange(e) { setAreaTag(e.target.value); setPage(1); fetchData({ reset: true }); }
  function onIssueChange(e) { setIssueTag(e.target.value); setPage(1); fetchData({ reset: true }); }
  function onSearchChange(e) {
    const value = e.target.value; setQ(value);
    if (debounceRef.current) clearTimeout(debounceRef.current);
    debounceRef.current = setTimeout(() => { setPage(1); fetchData({ reset: true }); }, 400);
  }
  function clearAll() { setQ(""); setAreaTag("all"); setIssueTag("all"); setPage(1); fetchData({ reset: true }); }
  ...
  async function callApi(url) {
    setApiOut("Loading…");
    try { const res = await fetch(url); const txt = await res.text(); setApiOut(txt); }
    catch (e) { setApiOut(`Error: ${e.message}`); }
  }
  ...
  return (
    <div style={{ fontFamily: "system-ui, sans-serif", padding: 24, background: "#f8fafc" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.3fr 0.7fr", gap: 16, alignItems: "start", maxWidth: 1200, margin: "0 auto" }}>
        {/* Search + Filters */}
        <div style={{ background: "#ffffff", padding: "12px 16px", borderRadius: 12, boxShadow: "0 4px 10px rgba(0,0,0,0.05)" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr 1fr auto", gap: 12 }}>
            <div>
              <label style={{ fontSize: 14, fontWeight: 600, display: "block", marginBottom: 6 }}>Search (title, summary, tags)</label>
              <div style={{ display: "flex", gap: 8 }}>
                <input value={q} onChange={onSearchChange} placeholder="e.g., transfer credit, Title IV, retention"
                       style={{ flex: 1, padding: "8px 10px", borderRadius: 8, border: "1px solid #e2e8f0", background: "#f1f5f9" }} />
                <button onClick={clearAll} style={{ padding: "8px 12px", borderRadius: 8, border: 0, background: "#e2e8f0" }}>Clear all</button>
              </div>
            </div>

            <label style={{ fontSize: 14, fontWeight: 600 }}>
              Area
              <select onChange={onAreaChange} value={areaTag}
                      style={{ marginLeft: 10, padding: "6px 10px", borderRadius: 8, border: "1px solid #e2e8f0", background: "#f1f5f9", minWidth: 200 }}>
            ...
            <label style={{ fontSize: 14, fontWeight: 600 }}>
              Issue
              <select onChange={onIssueChange} value={issueTag}
                      style={{ marginLeft: 10, padding: "6px 10px", borderRadius: 8, border: "1px solid #e2e8f0", background: "#f1f5f9", minWidth: 220 }}>
                <option value="all">All Issues</option>
                {issueTags.map((t) => (<option key={t} value={t}>{t.replace("issue_", "").replace(/_/g, " ")}</option>))}
              </select>
            </label>

            <div style={{ fontSize: 13, opacity: 0.8 }}>{total === null ? "" : `${total} result${total === 1 ? "" : "s"}`}</div>
          </div>
        </div>
        ...
        <div style={{ background: "#ffffff", padding: "12px 16px", borderRadius: 12, boxShadow: "0 4px 10px rgba(0,0,0,0.05)" }}>
          ...
          <div style={{ border: "1px solid #e2e8f0", borderRadius: 10, padding: 12, marginBottom: 12 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              <input value={ipedsUnitid} onChange={(e) => setIpedsUnitid(e.target.value)} placeholder="unitid (e.g., 217156)"
                     style={{ flex: 1, padding: 8, border: "1px solid #e2e8f0", borderRadius: 8 }} />
              <button onClick={() => callApi(`/.netlify/functions/fetchIPEDS?unitid=${encodeURIComponent(ipedsUnitid)}`)}
          ...
          <div style={{ border: "1px solid #e2e8f0", borderRadius: 10, padding: 12, marginBottom: 12 }}>
            <strong>BLS</strong>
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              <input value={blsSeries} onChange={(e) => setBlsSeries(e.target.value)} placeholder="series id (e.g., CES6562140001)"
                     style={{ flex: 1, padding: 8, border: "1px solid #e2e8f0", borderRadius: 8 }} />
              <button onClick={() => callApi(`/.netlify/functions/fetchBLS?series=${encodeURIComponent(blsSeries)}`)}
                      style={{ padding: "8px 12px", borderRadius: 8, border: 0, background: "#04143C", color: "#fff" }}>
                Fetch BLS
              </button>
            </div>
          ...
          <div style={{ border: "1px solid #e2e8f0", borderRadius: 10, padding: 12 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              <input value={cfrQuery} onChange={(e) => setCfrQuery(e.target.value)} placeholder="keyword (e.g., Title IV)"
                     style={{ flex: 1, padding: 8, border: "1px solid #e2e8f0", borderRadius: 8 }} />
              <button onClick={() => callApi(`/.netlify/functions/fetchCFR?query=${encodeURIComponent(cfrQuery)}`)}
                      style={{ padding: "8px 12px", borderRadius: 8, border: 0, background: "#04143C", color: "#fff" }}>
                Fetch CFR
              </button>
            </div>
          ...
          <pre style={{ marginTop: 12, background: "#0b1225", color: "#d2d6f3", padding: 12, borderRadius: 10, maxHeight: 240, overflow: "auto" }}>{apiOut || "API output will appear here…"}</pre>
        </div>
      </div>
          ...
          {records.map((r) => {
            ...
            return (
              <div key={r.id} style={{ border: "1px solid #e2e8f0", borderRadius: 12, padding: 16, background: "#ffffff" }}>
                <h3 style={{ marginTop: 0, marginBottom: 4, color: "#1e293b" }}>{highlight(r.title, q)}</h3>
                <p style={{ marginTop: 0, marginBottom: 8, fontSize: 14, color: "#334155" }}>{highlight(r.summary, q)}</p>
                {tags.length > 0 && (
